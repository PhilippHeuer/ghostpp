...
void CMap :: Load( CConfig *CFG, string nCFGFile )
{
	...
	{
		BOOST_LOG_TRIVIAL(info) << "[MAP] loading MPQ file [" + MapMPQFileName + "]";
		MapMPQReady = true;
	}
	else
		BOOST_LOG_TRIVIAL(info) << "[MAP] warning - unable to load MPQ file [" + MapMPQFileName + "]";
	...
	if( !m_MapData.empty( ) )
	{
		...
		BOOST_LOG_TRIVIAL(info) << "[MAP] calculated map_size = " + UTIL_ByteArrayToDecString( MapSize );
	...
	if( !m_MapData.empty( ) )
	{
		if( MapMPQReady )
		{
			...
			if( SFileOpenFileEx( MapMPQ, "war3map.w3i", 0, &SubFile ) )
			{
				...
				if( FileLength > 0 && FileLength != 0xFFFFFFFF )
				{
					...
					if( SFileReadFile( SubFile, SubFileData, FileLength, &BytesRead, NULL ) )
					{
						...
						if( FileFormat == 18 || FileFormat == 25 )
						{
							...
							BOOST_LOG_TRIVIAL(info) << "[MAP] calculated map_options = " + UTIL_ToString( MapOptions );
							MapWidth = UTIL_CreateByteArray( (uint16_t)RawMapWidth, false );
							BOOST_LOG_TRIVIAL(info) << "[MAP] calculated map_width = " + UTIL_ByteArrayToDecString( MapWidth );
							MapHeight = UTIL_CreateByteArray( (uint16_t)RawMapHeight, false );
							BOOST_LOG_TRIVIAL(info) << "[MAP] calculated map_height = " + UTIL_ByteArrayToDecString( MapHeight );
							MapNumPlayers = RawMapNumPlayers - ClosedSlots;
							BOOST_LOG_TRIVIAL(info) << "[MAP] calculated map_numplayers = " + UTIL_ToString( MapNumPlayers );
							MapNumTeams = RawMapNumTeams;
							BOOST_LOG_TRIVIAL(info) << "[MAP] calculated map_numteams = " + UTIL_ToString( MapNumTeams );

							uint32_t SlotNum = 1;

							for( vector<CGameSlot> :: iterator i = Slots.begin( ); i != Slots.end( ); ++i )
							{
								BOOST_LOG_TRIVIAL(info) << "[MAP] calculated map_slot" + UTIL_ToString( SlotNum ) + " = " + UTIL_ByteArrayToDecString( (*i).GetByteArray( ) );
								++SlotNum;
							}

							if( MapOptions & MAPOPT_MELEE )
							{
								BOOST_LOG_TRIVIAL(info) << "[MAP] found melee map, initializing slots";
						...
						BOOST_LOG_TRIVIAL(warning) << "[MAP] unable to calculate map_options, map_width, map_height, map_slot<x>, map_numplayers, map_numteams - unable to extract war3map.w3i from MPQ file";
				...
				BOOST_LOG_TRIVIAL(warning) << "[MAP] unable to calculate map_options, map_width, map_height, map_slot<x>, map_numplayers, map_numteams - couldn't find war3map.w3i in MPQ file";
		}
		else
			BOOST_LOG_TRIVIAL(warning) << "[MAP] unable to calculate map_options, map_width, map_height, map_slot<x>, map_numplayers, map_numteams - map MPQ file not loaded";
	}
	else
		BOOST_LOG_TRIVIAL(warning) << "[MAP] no map data available, using config file for map_options, map_width, map_height, map_slot<x>, map_numplayers, map_numteams";
	...
	{
		BOOST_LOG_TRIVIAL(info) << "[MAP] overriding calculated map_size with config value map_size = " + CFG->GetString( "map_size", string( ) );
		MapSize = UTIL_ExtractNumbers( CFG->GetString( "map_size", string( ) ), 4 );
	}
	...
	{
		BOOST_LOG_TRIVIAL(info) << "[MAP] overriding calculated map_info with config value map_info = " + CFG->GetString( "map_info", string( ) );
		MapInfo = UTIL_ExtractNumbers( CFG->GetString( "map_info", string( ) ), 4 );
	}
	...
	{
		BOOST_LOG_TRIVIAL(info) << "[MAP] overriding calculated map_crc with config value map_crc = " + CFG->GetString( "map_crc", string( ) );
		MapCRC = UTIL_ExtractNumbers( CFG->GetString( "map_crc", string( ) ), 4 );
	}
	...
	{
		BOOST_LOG_TRIVIAL(info) << "[MAP] overriding calculated map_sha1 with config value map_sha1 = " + CFG->GetString( "map_sha1", string( ) );
		MapSHA1 = UTIL_ExtractNumbers( CFG->GetString( "map_sha1", string( ) ), 20 );
	}
	...
	{
		BOOST_LOG_TRIVIAL(info) << "[MAP] overriding calculated map_filter_type with config value map_filter_type = " + CFG->GetString( "map_filter_type", string( ) );
	...
	{
		BOOST_LOG_TRIVIAL(info) << "[MAP] overriding calculated map_options with config value map_options = " + CFG->GetString( "map_options", string( ) );
		MapOptions = CFG->GetInt( "map_options", 0 );
	}
	...
	{
		BOOST_LOG_TRIVIAL(info) << "[MAP] overriding calculated map_width with config value map_width = " + CFG->GetString( "map_width", string( ) );
		MapWidth = UTIL_ExtractNumbers( CFG->GetString( "map_width", string( ) ), 2 );
	}
	...
	{
		BOOST_LOG_TRIVIAL(info) << "[MAP] overriding calculated map_height with config value map_height = " + CFG->GetString( "map_height", string( ) );
		MapHeight = UTIL_ExtractNumbers( CFG->GetString( "map_height", string( ) ), 2 );
	}
	...
	{
		BOOST_LOG_TRIVIAL(info) << "[MAP] overriding calculated map_numplayers with config value map_numplayers = " + CFG->GetString( "map_numplayers", string( ) );
		MapNumPlayers = CFG->GetInt( "map_numplayers", 0 );
	}
	...
	{
		BOOST_LOG_TRIVIAL(info) << "[MAP] overriding calculated map_numteams with config value map_numteams = " + CFG->GetString( "map_numteams", string( ) );
		MapNumTeams = CFG->GetInt( "map_numteams", 0 );
	}
	...
	{
		BOOST_LOG_TRIVIAL(info) << "[MAP] overriding slots";
		Slots.clear( );
	...
	if( m_MapFlags & MAPFLAG_RANDOMRACES )
	{
		BOOST_LOG_TRIVIAL(info) << "[MAP] forcing races to random";
	...
	if( m_MapObservers == MAPOBS_ALLOWED || m_MapObservers == MAPOBS_REFEREES )
	{
		...
		BOOST_LOG_TRIVIAL(info) << "[MAP] adding " + UTIL_ToString( MaxSlots - m_Slots.size( ) ) + " observer slots";
...
void CMap :: CheckValid( )
{
	...
	{
		m_Valid = false;
		BOOST_LOG_TRIVIAL(warning) << "[MAP] invalid map_path detected";
	}
	else if( m_MapPath[0] == '\\' )
		BOOST_LOG_TRIVIAL(warning) << "[MAP] map_path starts with '\\', any replays saved by GHost++ will not be playable in Warcraft III";

	if( m_MapPath.find( '/' ) != string :: npos )
		BOOST_LOG_TRIVIAL(warning) << "[MAP] map_path contains forward slashes '/' but it must use Windows style back slashes '\\'";

	if( m_MapSize.size( ) != 4 )
	{
		m_Valid = false;
		BOOST_LOG_TRIVIAL(warning) << "[MAP] invalid map_size detected";
	}
	else if( !m_MapData.empty( ) && m_MapData.size( ) != UTIL_ByteArrayToUInt32( m_MapSize, false ) )
	{
		m_Valid = false;
		BOOST_LOG_TRIVIAL(warning) << "[MAP] invalid map_size detected - size mismatch with actual map data";
	}
	...
	{
		m_Valid = false;
		BOOST_LOG_TRIVIAL(warning) << "[MAP] invalid map_speed detected";
	}
	...
	{
		m_Valid = false;
		BOOST_LOG_TRIVIAL(warning) << "[MAP] invalid map_visibility detected";
	}
	...
	{
		m_Valid = false;
		BOOST_LOG_TRIVIAL(warning) << "[MAP] invalid map_observers detected";
	}
	...
	{
		m_Valid = false;
		BOOST_LOG_TRIVIAL(warning) << "[MAP] invalid map_width detected";
	}

	if( m_MapHeight.size( ) != 2 )
	{
		m_Valid = false;
		BOOST_LOG_TRIVIAL(warning) << "[MAP] invalid map_height detected";
	}

	if( m_MapNumPlayers == 0 || m_MapNumPlayers > MAX_SLOTS )
	{
		m_Valid = false;
		BOOST_LOG_TRIVIAL(warning) << "[MAP] invalid map_numplayers detected";
	}

	if( m_MapNumTeams == 0 || m_MapNumTeams > MAX_SLOTS )
	{
		m_Valid = false;
		BOOST_LOG_TRIVIAL(warning) << "[MAP] invalid map_numteams detected";
	}

	if( m_Slots.empty( ) || m_Slots.size( ) > MAX_SLOTS )
	{
		m_Valid = false;
		BOOST_LOG_TRIVIAL(warning) << "[MAP] invalid map_slot<x> detected";
	}
}
