...
string UTIL_FileRead( string file, uint32_t start, uint32_t length )
{
	...
	if( IS.fail( ) )
	{
		BOOST_LOG_TRIVIAL(warning) << "[UTIL] warning - unable to read file part [" + file + "]";
		return string( );
	}
...
string UTIL_FileRead( string file )
{
	...
	if( IS.fail( ) )
	{
		BOOST_LOG_TRIVIAL(warning) << "[UTIL] warning - unable to read file [" + file + "]";
		return string( );
	}
...
bool UTIL_FileWrite( string file, unsigned char *data, uint32_t length )
{
	...
	if( OS.fail( ) )
	{
		BOOST_LOG_TRIVIAL(warning) << "[UTIL] warning - unable to write file [" + file + "]";
		return false;
	}
